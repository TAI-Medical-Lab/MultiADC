...
if __name__ == '__main__':
    ...
    fi=['rand10_smiles.csv','molecules.csv']
    dir_output = 'dataset/processed_aug/'
    os.makedirs(dir_output, exist_ok=True)

    smiles_set=set()
    for f in fi:

        dataset_data = pd.read_csv(f)

        s = dataset_data['sequence'].values
        # s = [str(item).upper() for item in s]
        # s = [str(item).upper() for item in s]
        s = s.tolist()
        linker_set = set(s)
        smiles_set = smiles_set.union(linker_set)

        # playload = dataset_data['playload'].values
        # linker = dataset_data['linker'].values
        # playload_set = set(playload.tolist())
        # linker_set = set(linker.tolist())
        # smiles_set = smiles_set.union(linker_set)
        # smiles_set = smiles_set.union(playload_set)
